...
copernicus_wind <- rast("cop_small.grib")
copernicus_wind

nlyr(copernicus_wind)
names(copernicus_wind)
ext(copernicus_wind)
crs(copernicus_wind)

first_layer <- copernicus_wind[[1]]
first_layer
crs(first_layer) <- crs(cropped_coast)
crs(first_layer) <- "epsg:4326"
extent <- ext(first_layer)

crs(first_layer)

cropped_coast <- st_read(dsn = "./processed_EvD/intersection_crop.shp")
ext <- ext(cropped_coast)
ext
cropped_cop <- crop(first_layer, cropped_coast)
res(first_layer)

# cropping the grib to james bay extent
jb_ext <- ext(-82,-75,51,55)
jb_poly <- as.polygons(jb_ext, crs = "epsg:4326")
jb_poly_proj <- project(jb_poly, crs(copernicus_wind))
crop_jb <- crop(copernicus_wind, jb_poly_proj)

# trying this with data from Canada's wind atlas instead
west_wind <- st_read("west_wind_data/ANU/9/16_30_we_dos.mif")
plot(west_wind)
head(west_wind)
west_wind <- as.ti(west_wind)
...
west_wind_filter <- west_wind %>% 
  dplyr::select(EU_12011_CAN___, MG_0_CAN1D068, LA_1_LATITUDE, LO_1_LONGITUD, geometry) %>% 
  dplyr::rename(mean_wind_speed_ms = EU_12011_CAN___, 
         land_water_mask= MG_0_CAN1D068, 
         lat = LA_1_LATITUDE, 
         long = LO_1_LONGITUD) %>% 
  dplyr::filter(land_water_mask == 0) %>% 
  dplyr::mutate(long_corrected = long-360)


plot(west_wind_filter)

# let's do the same with the east coast data
east_wind <- st_read("east_wind_data/ANU/9/18_30_we_dos.mif")
head(east_wind)

east_wind_filter <- east_wind %>% 
  dplyr::select(EU_12011_CAN___, MG_0_CAN1D068, LA_1_LATITUDE, LO_1_LONGITUD) %>% 
  dplyr::rename(mean_wind_speed_ms = EU_12011_CAN___, 
                land_water_mask= MG_0_CAN1D068, 
                lat = LA_1_LATITUDE, 
                long = LO_1_LONGITUD) %>% 
...
# write them as csvs to see them in GIS

write_csv(east_wind_filter, "east_wind_speed.csv")
write_csv(west_wind_filter, "west_wind_speed.csv")

#st_write(obj = west_wind_filter, 
         #dsn = "./", 
         #layer = "west_wind_shp",
